...
def appointment(request):
    ...
    if request.method == 'POST' and request.user.is_authenticated:
        ...
        # Basic validation
        if all([first_name, last_name, email, phone, specialty_id, appointment_date, appointment_time, reason]):
            # Here you would typically save the appointment to the database
            # For now, we'll just show a success message
            messages.success(request, 'Your appointment request has been submitted successfully! We will contact you soon to confirm.')
            return redirect('appointment')
        else:
            messages.error(request, 'Please fill in all required fields.')

    return render(request, 'appointment.html', {'doctors': doctors, 'specialties': specialties})
  ...
  if request.method == 'POST':
    ...
    if user is not None:
      ...
      if user.is_doctor:
        ...
        return redirect('home')  # Default to home page
    else:
